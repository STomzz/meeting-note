# 会议笔记（P8：笔记与会议合并为一栏）

一个栏目管全部：**vault 里的每篇 md 都是一篇笔记**，会议只是其中一类。
左侧目录树按文件夹组织（点开文件夹才看到笔记），开会时随手写、用 `/v` 引用录音，
会后点「一键处理」自动转写引用的音频并把「手写记录 + 转写」交给对话模型，在文件末尾生成纪要。
音频、文本、纪要全部在本地，云端只提供模型能力（ASR + 对话模型）。

## 长什么样

````md
# 2026-09-22 周会

> 2026-09-22 会议记录：随手写下要点；用 `/v 音频文件名` 引用录音。

## 我的记录

张三说镜像拉取超时，先换镜像站解决。
/v 会议音频/会议/2026-09-22-周会/20260922-0930/   ← 一次录音 = 一个场次目录 = 一条长语音
> 🎙 转写 00:00:00–00:52:30 · 13 段 · 20260922-0930  ← 处理时自动生成/覆盖（各段时间戳接成一条）
>
> [00:00:00] 张三：镜像拉取超时的问题……
> [00:04:11] 我：那就先换镜像站。

我又补了一句：下周把部署文档也补上。
/v 会议音频/会议/2026-09-22-周会/20260922-0930/   ← 同一个场次再插一次也只占一行

## 会议纪要（AI 整理）                        ← 这一行到文件末尾：处理时整段重写
### 结论
- 镜像拉取改用镜像站（张三）
- 下周补部署文档（我）
````

## 约定（人可读、幂等，不藏隐形标记）

| 约定 | 说明 |
| --- | --- |
| `/v <路径>` 或 `/video <路径>` **独占一行**（允许前导空白） | 音频引用；路径相对 vault，也可以只写文件名（会去 `会议音频/` 下按名字找） |
| 路径**行尾带 `/`**（或指向一个已存在的目录） | **整场引用**：目录里所有 `seg_*.wav` 按序号当**一条长语音**（播放器连播、转写各段时间戳接成一条、只占一行）。推荐用「一次录音 = 一个场次目录」的新写法和「插入整场」按钮 |
| 引用行**紧跟其后**的连续 `>` 块 | **程序生成**的转写，重新处理时整块替换（你自己的备注写在引用行上方） |
| `## 会议纪要（AI 整理）`（**取最后一次出现**） | 分界线，从这里到文件末尾由 LLM 生成并整段重写 |
| 手写内容 | 除了插入转写块与纪要段，程序永远不改你的文字 |

一键处理做的事：

1. 扫出所有 `/v` 引用并解析成文件或**场次目录**（拒绝绝对路径与 `..` 逃逸；找不到的记一条错误继续）；
2. 展开成一个个分段后逐段转写（静音切段 + 串行限流重试；进度按**段**算），
   **先写回转写块**（即使后面纪要失败，转写也已落盘）——整场引用写成一个块，
   各段时间戳按累计时长偏移接成一条时间轴，标题行形如 `🎙 转写 00:00:00–00:52:30 · 13 段 · 20260922-0930`；
3. 用「手写正文（去掉引用行、转写块、旧纪要）+ 所有转写」生成纪要，替换纪要段；
4. 手写记录与转写冲突时**以手写记录为准**（提示词里明确写了）。

## 数据在哪里

| 内容 | 位置 |
| --- | --- |
| 笔记（含会议） | 笔记库里任意路径；文件夹树由 vault 真实目录结构生成（隐藏目录与 `会议音频/` 除外） |
| 录音分段（WAV） | **vault 内** `会议音频/<笔记路径>/<场次>/seg_0001.wav`，如 `会议/周会.md` → `会议音频/会议/周会/20260922-0930/`；场次 = 一次「开始 → 停止」的记录（目录名 = 本机时间 `YYYYMMDD-HHMM`），与笔记一一对应、一眼可辨；跟着笔记一起备份/导出 |
| 旧版录音（0.1.x） | 仍在扁平目录 `会议音频/<会议名>/seg_0001.wav`（没有场次子目录，视作「早期录音」一场）；列表会**同时**列出新旧目录，新的写进场次目录，历史引用不受影响 |
| 转写缓存 | `<应用数据目录>/index.sqlite` 表 `audio_transcripts`（键：路径 + mtime + size + ASR 模型） |
| 旧版会议（迁移用） | 旧录音仍在 `<应用数据目录>/meetings/<会议ID>/`，导出时**复制**进 vault |

应用数据目录（桌面端）：

- Windows：`%APPDATA%\com.bnu.notes\`
- Linux/macOS：`~/.local/share/com.bnu.notes/`（macOS 为 `~/Library/Application Support/com.bnu.notes/`）
- Android：应用私有目录

**体积**：16 kHz 单声道 WAV ≈ 1.9 MB/分钟，4 分钟一段 ≈ 7.7 MB。录音直接在 vault 里，
以后 zip 导出笔记时注意体积（可以只导出 md、不导出 `会议音频/`）。

## 录音

- 入口：右下角浮动 🎙 按钮（任意页面可用）。展开后是**一行小条**（236px 宽，v0.3.6 起把原来的大方卡压小，
  尽量不挡正文）：待录时「记到哪篇笔记 ▾ ＋ 开始」，录音中「红点 ＋ 计时 ＋ 电平 ＋ 停止」，
  第二行 11px 轻文字给段号 / 存到哪篇；选定目标 → 开始录音 → 4 分钟自动切段。
- **一次录音 = 一个场次目录**（v0.3.7）：开始录音时建 `会议音频/<笔记>/<YYYYMMDD-HHMM>/`，
  4 分钟切一段仍写在这个目录里（`seg_0001.wav`、`seg_0002.wav`…）。插入笔记时写的是
  **目录引用一行** `/v 会议音频/<笔记>/<场次>/`，播放器把它当**一条长语音**连播——
  笔记里不再是一段一行。
- **停止只保存文件，不自动写引用**：面板提示「本场已存 N 段」，「插入整场」按一下就把这一场的
  目录引用插到光标处（笔记没打开时追加到正文末尾、纪要标题之前）。
- 编辑器工具栏「🎙 N 段」里按**场次**分组列出本笔记的录音：场次行显示 `N 段 · 总时长 · 已引用整场/未引用`，
  带「插入整场」；展开的分段行仍可试听、单独插入（单段引用）、丢弃（二次确认）。
  「插入全部未引用」按场次合并成一行一场。
- **每段结束就自动转写**（v0.3.7，默认开，设置页 →「录音与转写」可关）：4 分钟切段或停止录音时，
  刚收尾的那一段会立刻进后台**串行**队列，走与「一键处理」相同的链路（静音切段 + 限流 + 重试），
  **只写转写缓存、不动笔记**；录音面板显示「转写 N」进度与失败数。这样之后点「一键处理」
  时这些段全部命中缓存，只剩纪要生成（快很多）。一键处理前会先等队列排空，避免两条链路抢上游额度。
- 一键处理前如果目录里还有未引用的录音，会弹窗提示，可选择「插入并处理 / 忽略并继续 / 取消」。
- 采集：Web Audio `getUserMedia` → `ScriptProcessorNode` 取 Float32 → 转 PCM16 单声道。
  不用 `MediaRecorder`：上游 ASR 只吃 WAV，而 WebView 的 `MediaRecorder` 在 WebKitGTK 上不稳定。
- 采样率：优先请求 16 kHz，设备不支持时按设备采样率采集（常见 48 kHz），
  **按原采样率保存**（保真回放），送 ASR 前由 Rust 侧重采样到 16 kHz。
- 落盘：每 1 秒把 PCM 追加写入当前分段，Rust 侧先写好 WAV 头、关闭分段时回写长度。
- 分段上限：`SEGMENT_MAX_MS = 4 * 60 * 1000`（`src/stores/meetingNote.ts`）。
- Android v1：只做前台录音；录音期间申请 `screen` Wake Lock 保持屏幕常亮（失败不阻断）。
- 录坏了：编辑器工具栏「🎙 N 段」里「丢弃」（二次确认）删掉该分段文件——已写进笔记的引用要自己删。

### 平台麦克风权限

| 平台 | 需要什么 |
|---|---|
| Windows | ① 应用内已注册 WebView2 `PermissionRequested`，只放行本应用页面（`http(s)://tauri.localhost` / 开发服务器）的麦克风/摄像头，见 `src-tauri/src/webview_permissions.rs`；② 系统「设置 → 隐私和安全性 → 麦克风」需允许桌面应用访问麦克风，否则 `getUserMedia` 直接 `NotAllowedError` |
| Android | 系统运行时权限弹窗（Manifest 已声明 `RECORD_AUDIO`），拒绝后需到系统设置里重开；开着「自动转写」时录音期间会持续上传音频，流量敏感时可以先去设置页关掉 |
| Linux (WebKitGTK) | 未处理 WebKitGTK 媒体权限（WSL 无麦克风，无法实测） |

## 文件夹与整理（P8）

左侧是 vault 的真实目录树：**点开文件夹才看到里面的笔记**，文件夹后面的数字含子文件夹。

- 「新建笔记」可选文件夹；「新建文件夹」支持多级嵌套（输入名、选上级）。
- 右键文件夹：在此新建笔记 / 新建子文件夹 / 重命名文件夹。
- 右键笔记：重命名笔记 / 移动到…。
- 重命名文件夹：只改目录名。
- 重命名笔记 = 改标题（正文第一个 `#` 行）：文件名与标题一致时文件名同步改；
  `会议/2026-09-22-周会.md` 这类文件名带日期、标题是「周会」的，只改标题、文件名保持不动。
- 移动笔记只改位置；`/v` 引用是 vault 相对路径，不会失效。
  音频继续留在原目录（新录音会写进新路径对应的目录，新旧目录都会被列出）。
- v1 不做删除文件夹（避免误删录音），删除笔记仍在编辑器右上角（二次确认）。

## 一键处理（转写 + 纪要）

入口：编辑器工具栏「一键处理（N 段录音）」（有 `/v` 引用时才出现）；按钮带下拉，可选
「强制重新转写」= 忽略 `audio_transcripts` 缓存重新烧 ASR。
处理前若有未引用录音会先弹窗提示（插入并处理 / 忽略并继续 / 取消）。

送 ASR 前会在 Rust 侧做**静音切段**（减少长音频的超时/显存压力）：

| 参数 | 值 |
| --- | --- |
| 采样率 | 16 kHz 单声道 16 bit |
| 分析帧长 | 100 ms |
| 切段条件 | 静音 ≥ 600 ms 且本段 ≥ 8 s |
| 强制切段 | 45 s |
| 收尾 | 残余 < 1 s 丢弃，≥ 1 s 保留 |
| 静音判定 | 自适应噪声地板 `max(220, floor×2.5)` |

请求编排：

- 串行发送 + 令牌桶限流 **9 请求/分钟**（图谱抽取与转写共用同一套限流器，见 `rate_limit.rs`）；
- 失败重试 3 次：`429` → 15 s×次数，其它网络错误 → 2 s×次数；确定性 4xx 直接判失败；
- 进度走 `meeting-note-progress` 事件（`phase`：parse / transcribe / write / minutes / done），
  可以随时「取消」（当前请求结束后停下，已完成的转写会落盘）；
- **转写缓存**：同一音频（路径 + mtime + size + ASR 模型 都一样）第二次处理直接复用，
  不重复烧配额；重跑时笔记里的转写块不会堆叠（幂等替换）。自动转写（录音中）写的就是这份缓存，
  所以「一键处理」通常只剩纪要生成；
- 进度与结果都按**分段**计数（`audio_total` = 要转写的段数）：整场引用里哪一段失败，
  写回的转写块开头会有一行「（N 段转写失败：seg_xxxx.wav；可再点一次「一键处理」只重试这些）」；
- 单段失败不影响其它段：失败的那条错误写进结果里，其余照常写回与生成纪要。

纪要：

- 提示词在 `crates/bnu-core/src/minutes.rs`：旧流程用 `MINUTES_SYSTEM_PROMPT`，
  会议笔记流程用 `notes_system_prompt()`（在铁律之外补充「手写记录优先于转写」）；
- 优先带 `response_format: {"type":"json_object"}`；上游 400 时自动降级重试一次；
- JSON 解析失败会自动「修复重试」一次；
- 内容 ≥ 60 000 字先 map（20 000 字分片）再 reduce；
- 结构化字段：`title / meeting_date / participants / overview / topics[] / decisions[] /
  action_items[] / risks[] / next_steps[]`，渲染成 Markdown 时待办是表格。

## 旧会议迁移

设置页 → 数据维护 → 「把旧版会议导出为笔记」（一次性，可重复点但已存在的会跳过）：

- 旧录音从 `<应用数据>/meetings/<ID>/seg_*.wav` **复制**（不移动、不删除）到
  `会议音频/旧会议-<标题>/`，并在笔记里生成 `/v` 引用；
- 旧转写按 `[hh:mm:ss]` 时间戳分摊到各分段，放在对应引用下面（识别不出时间戳就整体放第一段；
  没有录音文件就退化成 `## 旧版转写` 小节）；
- 旧纪要写进 `## 会议纪要（AI 整理）` 段；目标笔记已存在则跳过，不覆盖。

导出后旧会议记录（`meetings` / `meeting_segments` 表）保留不动——**没有删除**，
旧录音目录也还在，确认迁移没问题之后要不要清理由你决定。

## 接线（客户端命令）

| 命令 | 作用 |
| --- | --- |
| `list_folders` / `create_folder` / `rename_folder` | 文件夹树（含空文件夹）/ 新建（多级）/ 重命名 |
| `move_note` / `rename_note` | 移动笔记 / 重命名笔记（重名自动加序号） |
| `meeting_notes_list`（`scanAll`） | 会议列表兼容接口（旧版会议页用；新界面直接用 `list_notes` + 录音统计） |
| `meeting_note_new` | 新建 `会议/<日期>-<标题>.md`（带骨架 + 纪要段） |
| `meeting_note_refs` | 解析一篇笔记里的引用（`kind`：file/session、分段清单、总时长、是否已有转写） |
| `meeting_note_process` / `meeting_note_cancel` | 一键处理（事件 `meeting-note-progress` / `meeting-note-processed`） |
| `meeting_note_migrate_legacy` | 旧会议导出为笔记 |
| `audio_clip_start`（noteId + session + 采样率） | 开始一次录音：建场次目录（`会议音频/<笔记>/<YYYYMMDD-HHMM>/`），返回目录、序号与两种引用行 |
| `audio_clip_append` / `audio_clip_close`（dir + seq） | 分段落盘 / 收尾（PCM 走 base64，避免大数组 JSON） |
| `audio_clip_transcribe`（按路径） | 单段转写（录音过程中「每段结束就转」用）：只写转写缓存，不改笔记 |
| `audio_clip_discard`（按路径） | 丢弃一个分段（UI 二次确认） |
| `audio_clip_list` | 列出 vault 内录音（含 `session`）；带 `noteId` 时只列该笔记（新目录 + 其下所有场次 + 旧版目录） |
| `read_note` / `write_note` | 笔记读写（会议笔记也是普通笔记） |

播放：音频在 vault 里，走 Tauri 的 `asset://` 协议；**vault 目录在启动与切换 vault 时
动态加入 asset scope**（`app.asset_protocol_scope().allow_directory`），
静态 scope（`tauri.conf.json`）里只有应用数据目录。

## 预览模式

浏览器里 `npm run dev`（非 Tauri 壳）时走内存实现：一篇示例会议笔记 + 假转写/假纪要。
示例笔记带一个场次引用，播放器**可以真跑**（mock 会给每段造一段 1.5 秒静音 WAV 的 blob 地址，
连播、拖动、段号都能验）；真实录音、转写、纪要请在桌面客户端验证。

## 排障

| 现象 | 排查 |
| --- | --- |
| 「未配置语音转写模型」 | 设置页填 ASR 端点（`qwen3-asr-1.7b` 这类） |
| 「未配置对话模型，已跳过纪要生成」 | 转写已保存；到设置页填对话端点后重跑 |
| 找不到音频「xxx」 | 笔记里的路径写错或文件被移走；`/v` 只写文件名时会去 `会议音频/` 下找 |
| 处理很慢 | 每段音频要过静音切段 + 限流 9 请求/分钟；面板会显示「限流等待 Ns」 |
| 转写是空的 | 音频几乎全是静音（切段后无内容）；换掉这段或删掉引用 |
| 纪要里出现转写没有的内容 | 检查是否把旧纪要/其它文本写进了正文；提示词已要求「只依据原文」 |
| 播放器不出声 | vault 目录没进 asset scope（换 vault 后应自动放行）或文件缺失 |
| 整场播放器显示「这场的分段不在」 | 场次目录里的 `seg_*.wav` 被删/被移走（丢弃过头了）：重新录一场，或把段落再录一遍 |
| 笔记里还是一条条 `seg_0001.wav` | 那是 0.1.x 的老写法（仍能播、能转写）；0.3.8 会给老笔记一个「合并成长语音」按钮 |
| Android 录音没声音 | 设置页 → 数据维护 →「录音自检」→「检查环境」看 `getUserMedia` / Wake Lock 支持情况 |
| 处理完引用被删了 | 引用行紧跟的 `>` 块被视为程序生成（会整块替换）；自己的备注写到引用行上方 |

## 已知限制

- 无实时流式转写、无说话人分离（产品范围外）；
- 老笔记（一段一行引用）的「合并成长语音」还没做（计划 0.3.8：移动文件进一个场次目录 + 合成一行引用 + 合并转写块）；
- 场次目录名用的是**开始录音时的本机时间**，不是会议时间；同一个场次目录重复进（同一分钟又点开始）会接着已有序号排；
- 录音面板同时只能录一路（切换目标笔记前先停止）；
- 转写缓存按「文件戳 + 模型」命中，改内容不换文件会复用旧文本（可勾「强制重新转写」）；
- 纪要生成 completion token 上限 8192；「手写 + 转写」合计上限 150 万字（超出直接报错，不静默截断）；
- Android 音频焦点/中断（来电、其它 App 抢占麦克风）v1 不做处理，息屏或切后台可能中断录音；
- Linux 桌面端未处理 WebKitGTK 媒体权限，录音仅在 Windows / Android 验证；
- 实体同义归并、说话人分离、zip 同步导出都还没做（见各自文档/计划）。
