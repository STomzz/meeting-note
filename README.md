...
- 笔记编辑器：默认**所见即所得**（点段落即改，无输入框边框、只有光标闪；输入 `#` / `/` 换块类型、`/v` 插录音引用、
  回车拆段），只替换被编辑的那一块——会议笔记的 `/v` 引用、转写块、纪要段不会被重排；需要抠 markdown 原文时切「源码」。
- 录音：右下角胶囊/面板，录音中显示计时与电平，停止只保存分段（默认 4 分钟一段）不自动写引用；
  引用行右侧 `⋯` 可「复制路径 / 删除引用」（只删文档里的引用行，音频文件保留）。
